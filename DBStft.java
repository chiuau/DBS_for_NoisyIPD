/*
 * Derivative Belief Strategy (with learning prevention) (version 1.3) (Mar 2, 2005)
...
/**
 * Derivative Belief Strategy (with TFT as the first counter strategy)
 * @author Tsz-Chiu Au
 */
public class DBStft extends Strategy {
    private final static String _abbrName = "DBStft";
    private final static String _name = "DBS with TFT" ;
    ...
    private final static int exactHistoryLen = 20 ;
    ...
    private int countMyUnfairActYL1MN0(int age) {
      int i ;
      int c=0 ;
      int traceBackLen = (age<=exactHistoryLen)?age:exactHistoryLen ;

      for(i=0; i<traceBackLen; i++) {
        if (HexactYLmove[i]==1 && HexactMNmove[i]==0) { c++ ; }
      }

      return c ;
    }

    private int countMyUnfairActYL0MN1(int age) {
      int i ;
      int c=0 ;
      int traceBackLen = (age<=exactHistoryLen)?age:exactHistoryLen ;

      for(i=0; i<traceBackLen; i++) {
        if (HexactYLmove[i]==0 && HexactMNmove[i]==1) { c++ ; }
      }

      return c ;
    }

    private int countYourUnfairActML1YN0(int age) {
      int i ;
      int c=0 ;
      int traceBackLen = (age<=exactHistoryLen)?age:exactHistoryLen ;

      for(i=0; i<traceBackLen; i++) {
        if (HexactMLmove[i]==1 && HexactYNmove[i]==0) { c++ ; }
      }

      return c ;
    }

    private int countYourUnfairActML0YN1(int age) {
      int i ;
      int c=0 ;
      int traceBackLen = (age<=exactHistoryLen)?age:exactHistoryLen ;

      for(i=0; i<traceBackLen; i++) {
        if (HexactMLmove[i]==0 && HexactYNmove[i]==1) { c++ ; }
      }
    ...
    /* use TFT as a counter stragey if BS is like TFT */

    private int isBSLikeTFT() {
      int[][] combinedBS = new int[2][2] ;
      int count =0 ; 
      ...
      if (combinedBS[0][0]==0) { count++ ; }
      if (combinedBS[0][1]==1) { count++ ; }
      if (combinedBS[1][0]==0) { count++ ; }
      if (combinedBS[1][1]==1) { count++ ; }

      return (count >= 3)?1:0 ;
    }

    private int TFTFmove() {
      if (yourMove==1) {
        return 1 ;
      } else {
        if (Math.random()<0.2) {
    ...
    public double getMove() {
      ...
      if (isFirstMove==1) {
        ...
        if (isBSLikeTFT()==1) {
          ...
          if (countMyUnfairActYL1MN0(5)>=2 && yourMove==0 && m==1) { m=0 ; }
          if (countMyUnfairActYL0MN1(5)>=2 && yourMove==1 && m==0) { m=1 ; }
